...
class Tetris:
    ...
    def __init__(self, height=20, width=10, block_size=20):
        ...
        self.extra_board = np.ones((self.height * self.block_size, self.width * int(self.block_size / 2), 3),
                                   dtype=np.uint8) * np.array([255, 204, 229], dtype=np.uint8)
    ...
    def get_state_properties(self, board):
        ...
        variance = self.variance(board)

        return torch.FloatTensor([lines_cleared, holes, bumpiness, height, variance])
    ...
    def variance(self, board):
        board = np.array(board)
        mask = board != 0
        invert_heights = np.where(mask.any(axis=0), np.argmax(mask, axis=0), self.height)
        board_heights = self.height - invert_heights
        average = sum(board_heights) / float(self.width)
        variance = 0
    ...
    def calculate_reward(self, states):
        reward = []
        for state in states:
            reward.append(state[4])
            
    def calculate_reward_4h(self, states):
        rewards = []
    ...
    def render(self, video=None):
        ...
        cv2.putText(img, "Score: " + str(self.score), (self.width * self.block_size + int(self.block_size / 2), self.block_size),
                               fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.6, thickness=1, color=self.text_color)
               

        cv2.putText(img, "Pieces: " + str(self.tetrominoes), (self.width * self.block_size + int(self.block_size / 2), 4 * self.block_size),
                fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.6, thickness=1, color=self.text_color)


        cv2.putText(img, "Lines: " + str(self.cleared_lines), (self.width * self.block_size + int(self.block_size / 2), 7 * self.block_size),
                   fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.6, thickness=1, color=self.text_color)

        if video:
            video.write(img)
        print(self.cleared_lines)
        cv2.imshow("Trained Tetris Agent Display", img)
        cv2.waitKey(1)
